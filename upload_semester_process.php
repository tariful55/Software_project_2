...
if (!empty($clo_updates)) {
    $sql = "UPDATE ece2217 SET " . implode(", ", $clo_updates) . " WHERE roll = 0";
...
if (!empty($plo_updates)) {
    $sql = "UPDATE ece2217 SET " . implode(", ", $plo_updates) . " WHERE roll = 1";
    mysqli_query($conn, $sql);
}
...
if (!empty($full_updates)) {
    $sql = "UPDATE ece2217 SET " . implode(", ", $full_updates) . " WHERE roll = 2";
    mysqli_query($conn, $sql);
}
...
for ($i = 0; $i < $total_students; $i++) {
    ...
    if (!empty($updates)) {
        $sql = "UPDATE ece2217 SET " . implode(", ", $updates) . " WHERE roll = '$roll'";
        mysqli_query($conn, $sql);
    }
}
